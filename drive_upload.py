"""drive_upload.py — uploads generated report PDFs to Google Drive.

A cloud routine's filesystem does not persist between runs, so a PDF that
...
def upload_reports(pdf_paths: list, folder_id: str) -> list:
    """Upload each PDF to the given Drive folder.
    ...
    if not os.environ.get("GOOGLE_SERVICE_ACCOUNT_JSON"):
        print("GOOGLE_SERVICE_ACCOUNT_JSON not set — skipping Drive upload "
              "(PDFs remain only in ./reports)")
        return [{"path": p, "status": "skipped"} for p in pdf_paths]
    ...
    except Exception as e:  # noqa: BLE001 — bad/malformed credentials must fail each PDF cleanly, not crash the whole scheduled run
        print(f"FAILED to initialize Drive credentials: {e}")
        return [{"path": p, "status": "failed", "error": f"credentials error: {e}"} for p in pdf_paths]
